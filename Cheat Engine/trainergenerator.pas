...
type
  ...
  TfrmTrainerGenerator = class(TForm)
    ...
    cbSupportLocalAccess: TCheckBox;
    ...
    procedure cbSupportLocalAccessChange(Sender: TObject);
...
resourcestring
  ...
  rsTo = 'to';
  rsBy = 'by';
  rsOnCloseWarning = 'This form had an onClose event. Good thing this was only a stub, else '+strLocalAccess+' would have terminated';
  ...
  rsYouNeedACheatTableWithCheatEntries = 'You need a cheat table with cheat '
    +'entries';
  rsDonTSupportLocalAccessOrYourself = 'Don''t support '+strLocalAccess+' (or '
    +'yourself)';
...
begin
  ...
  if not cbSupportLocalAccess.checked then
    cbSupportLocalAccess.checked:=true;
...
begin
  ...
  l.add('  messageDialog(''Please install '+strLocalAccess+' ''..RequiredCEVersion, mtError, mbOK)');
  l.add('  closeCE()');
  l.add('end');
  ...
  try
    ...
    begin
      ...
      begin
        ...
        l.add('supportLocalAccess('+trainerform.name+', '+BoolToStr(adconfig.cbCanClose.checked,'true','false')+', '+adconfig.edtWidth.text+', '+adconfig.edtHeight.text+', '+inttostr(adconfig.adposition)+', '+QuotedStr(adconfig.ownurl)+', '+QuotedStr(adconfig.extraparam)+', '+inttostr(adconfig.percentage)+')');
...
begin
  cbSupportLocalAccess.caption:=rsDonTSupportLocalAccessOrYourself;
  if restoretimer<>nil then
    restoretimer.enabled:=false;
end;

procedure TfrmTrainerGenerator.cbSupportLocalAccessChange(Sender: TObject);
begin
  //Guilt procedure
  if not cbSupportLocalAccess.checked then
  begin
    cbSupportLocalAccess.caption:=rsThankYou;
  ...
  begin
    cbSupportLocalAccess.caption:=rsAaaaw;
    if adwindow<>nil then
    begin
