...
type
  TLuaSettings=class    //A wrapper for the registry object to make access to the local access settings easier and uniform
  private
    freg: Tregistry;
    fpath: string;
...
begin
  ...
  if freg.OpenKey('\Software\'+strLocalAccess+'\'+v, true) then
    fpath:=v;
end;
...
begin
  ...
  if initialpath<>nil then
    ...
    freg.OpenKey('\Software\'+strLocalAccess,true);
