...
resourcestring
  ...
  rsYouHavenTSelectedAnyMemoryTypeThisWillResultInChea = 'You haven''t selected any memory type. This will result in '+strLocalAccess+' finding NO memory! Are you stupid?';
  ...
  rsPopupHideLocalAccess = 'Popup/Hide '+strLocalAccess;
  ...
  rsRestartCE = 'It is recommended to restart '+strLocalAccess+' for this change to take effect';
...
begin
    ...
    try
      ...
      begin
        ...
        try
          ...
          reg.{$ifdef windows}WriteBinaryData{$else}WriteString{$endif}('Show '+strLocalAccess+' Hotkey',{$ifndef windows}bintohexs({$endif}frameHotkeyConfig.newhotkeys[1][0],10){$ifndef windows}){$endif};
        ...
        begin
          if reg.OpenKey('\Software\'+strLocalAccess+'\Font', true) then
  ...
  {$ifndef net}
      ...
      reg.DeleteKey('\Software\'+strLocalAccess+'\Tools');
      if Reg.OpenKey('\Software\'+strLocalAccess+'\Tools',true) then
      begin
      ...
      //save the plugins
      reg.DeleteKey('\Software\'+strLocalAccess+'\Plugins'+cpu);
      if Reg.OpenKey('\Software\'+strLocalAccess+'\Plugins'+cpu,true) then
      begin
...
begin
  ...
  begin
    ...
    try
      ini:=TIniFile.Create(localaccessdir+{$ifdef darwin}'..'+DirectorySeparator+{$endif}'Languages' + DirectorySeparator+'language.ini');
      try
...
begin
  ...
  {$ifdef darwin}
  OutputDebugString('ScanForLanguages: Looking in '+LocalAccessDir+{$ifdef darwin}PathDelim+'..'+{$endif}PathDelim+'Languages');
  {$endif}
  FindAllDirectories(f,LocalAccessDir+{$ifdef darwin}PathDelim+'..'+{$endif}PathDelim+'Languages',false);

  index:=1;
  for i:=0 to f.Count-1 do
  begin
    n:=f[i];
    if not (fileexists(n+pathsep+'localaccess.po') or fileexists(n+PathDelim+'localaccess-x86_64.po') or fileexists(n+PathDelim+'localaccess-i386.po')) then
      continue;
