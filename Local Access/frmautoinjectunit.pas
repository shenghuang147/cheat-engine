...
{
The TDisassemblyLine originates from jgoemat  ( http://forum.localaccess.org/viewtopic.php?t=566415 )
...
resourcestring
  ...
  rsCEAFilter = strLocalAccess+' Assembly (*.CEA)|*.CEA|All Files ( *.* )|*.*';
  rsCEGAFilter = strLocalAccess+' GNU Assembly (*.CEGA)|*.CEGA|All Files ( *.* )|*.*';
...
begin
  ...
  begin
    ...
    try
      if reg.OpenKey('\Software\'+strLocalAccess+'\Auto Assembler\',false) then
      begin
...
{$ifndef standalonetrainerwithassembler}
 ...
 obsolete
  ...
  begin
    ...
    injectdll(localaccessdir+'undercdll.dll','');
  ...
  begin
    try
      ...
      begin
        ...
        reg:=tregistry.create;
        try
          if reg.OpenKey('\Software\'+strLocalAccess+'\Auto Assembler\',true) then
          begin
...
// \/   http://forum.localaccess.org/viewtopic.php?t=566415 (jgoemat and some mods by db)
procedure GenerateFullInjectionScript(Script: tstrings; address: string; commentRadius: integer=10; farjmp: boolean=false; jmp1:boolean=false; originalcodeMinSize: integer=1);
var
  originalcode: tstringlist;
  originalbytes: array of byte;
...
begin
  ...
  // same as menu option "Local Access framework code", make sure we
...
begin
  LuaHighlighter.LoadFromRegistry(HKEY_CURRENT_USER, '\Software\'+strLocalAccess+'\Lua Highlighter'+darkmodestring);
  AAHighlighter.LoadFromRegistry(HKEY_CURRENT_USER, '\Software\'+strLocalAccess+'\AA Highlighter'+darkmodestring);
  CPPHighlighter.LoadFromRegistry(HKEY_CURRENT_USER, '\Software\'+strLocalAccess+'\CPP Highlighter'+darkmodestring);
end;
...
begin
  ...
  begin
    LuaHighlighter.SaveToRegistry(HKEY_CURRENT_USER, '\Software\'+strLocalAccess+'\Lua Highlighter'+darkmodestring);
    reloadHighlighterSettings;
...
begin
  ...
  if frmHighlighterEditor.showmodal=mrok then
  begin
    AAHighlighter.SaveToRegistry(HKEY_CURRENT_USER, '\Software\'+strLocalAccess+'\AA Highlighter'+darkmodestring);
...
begin
  frmHighlighterEditor:=TfrmHighlighterEditor.create(self);
  CPPHighlighter.loadFromRegistryDefault(HKEY_CURRENT_USER, '\Software\'+strLocalAccess+'\CPP Highlighter'+darkmodestring);

  frmHighlighterEditor.highlighter:=CPPHighlighter;
  if frmHighlighterEditor.showmodal=mrok then
  begin
    CPPHighlighter.SaveToRegistry(HKEY_CURRENT_USER, '\Software\'+strLocalAccess+'\CPP Highlighter'+darkmodestring);
    ReloadAllAutoInjectHighlighters;
  end;
